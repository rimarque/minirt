...
/*   Updated: 2024/02/26 18:10:03 by rita             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	in_cy_surface(t_ray ray, t_obj cy, t_auxeq *aux)
{
	...
	b = 2 * (vec3_dot(ray.d, cy.co) - aux->dot_dv * cy.dot_cov);
	//c = vec3_dot(co, co) - aux->dot_cov * aux->dot_cov - cy.r_sqr;
	if(!aply_quadratic_form(a, b, cy.qf_c, aux))
		return (false);
	return (true);
}
...
t_inter	get_inter(t_vec3 d, t_obj cy, float t1, float t2)
{
	t_inter	it1;
	t_inter it2;
	
	it1 = get_inter_tm(d, cy, cy.dot_cov, t1);
	it2 = get_inter_tm(d, cy, cy.dot_cov, t2);
...
t_inter	inter_surface(t_ray ray, t_obj cy)
{
	...
	if(!in_cy_surface(ray, cy, &aux) || (aux.t1 < 0 && aux.t2 < 0))
		return(it);
	if(aux.in_sqrt == 0)
		it = get_inter_tm(ray.d, cy, cy.dot_cov, aux.t1);
	else
		it = get_inter(ray.d, cy, aux.t1, aux.t2);
	if(it.inter == false)
			return(it);
